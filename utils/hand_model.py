...
class HandModel:
    def __init__(
        ...
        xml_bytes = Path(urdf_path).read_bytes()  # read as bytes
        self.pk_chain = pk.build_chain_from_urdf(xml_bytes).to(
        ...
        if os.path.exists(
            links_pc_path
        ):  # In case of generating robot links pc, the file doesn't exist.
        ...
        self.meshes = load_link_geometries(
            robot_name, self.urdf_path, self.pk_chain.get_link_names()
        )

        self.vertices = {}
        removed_links = json.load(
            open(os.path.join(ROOT_DIR, "data_utils/removed_links.json"))
        )[robot_name]
    ...
    def get_transformed_links_pc(self, q=None, links_pc=None):
        ...
        for link_index, (link_name, link_pc) in enumerate(links_pc.items()):
            ...
            link_pc_homogeneous = torch.cat(
                [link_pc.to(q.device), homogeneous_tensor], dim=1
            )
    ...
    def get_initial_q(self, q=None, max_angle: float = math.pi / 6):
        ...
        if q is None:  # random sample root rotation and joint values
            ...
            lower_joint_limits = torch.tensor(
                lower_joint_limits[6:], dtype=torch.float32
            )
            upper_joint_limits = torch.tensor(
                upper_joint_limits[6:], dtype=torch.float32
            )
            portion = random.uniform(0.65, 0.85)
            q_initial[6:] = lower_joint_limits * portion + upper_joint_limits * (
                1 - portion
            )
        else:
            if len(q) == self.dof:
            ...
            angle = torch.tensor(
                random.uniform(0, max_angle), device=q.device
            ...
            portion = random.uniform(0.65, 0.85)
            q_initial[9:] = lower_joint_limits * portion + upper_joint_limits * (
                1 - portion
            )
    ...
    def get_trimesh_q(self, q):
        ...
        for link_name in self.vertices:
            mesh_transform_matrix = (
                self.frame_status[link_name].get_matrix()[0].cpu().numpy()
            )
            scene.add_geometry(
                self.meshes[link_name].copy().apply_transform(mesh_transform_matrix)
            )
...
def create_hand_model(
    ...
    hand_model = HandModel(
        robot_name, urdf_path, meshes_path, links_pc_path, device, num_points
    )
    return hand_model
